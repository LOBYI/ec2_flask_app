from flask import Flask,render_template, request, render_template_string, redirect, session, url_for, g, send_file
import os,sqlite3,subprocess, urllib.request, base64
...
def board():
    ...
    if request.method == 'GET': # 처음 페이지가 로드되는 GET 통신
        ...
        return render_template('board.html', error=error, name=id, data_list=data_list, free_write=free_write, search=search) # html을 렌더하며 DB에서 받아온 값들을 넘김
